...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget buildFriendsButton({
    ...
    if (currentUser.uid == userModel.uid && userModel.friendsUIDs.isNotEmpty) {
      ...
      if (currentUser.uid != userModel.uid) {
        // show cancle friend request button if the user sent ui friend request
        ...
        if (userModel.friendRequestsUIDs.contains(currentUser.uid)) {
          ...
          return buildElevatedButton(
            onPressed: () async {
              await context
                  ...
                  .whenComplete(() {
                showSnackBar(context, 'friend request cancled');
              });
            },
          ...
          return Row(
            ...
            children: [
              buildElevatedButton(
                onPressed: () async {
                  // show unfriend dialog to ask the user if he is sure to unfriend the user
                        ...
                        TextButton(
                          onPressed: () async {
                            ...
                            await context
                                ...
                                .whenComplete(() {
                              showSnackBar(
                                  context, 'You are no lorger friends');
                              Navigator.pop(context);
                            });
                          },
